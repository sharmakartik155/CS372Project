...
<?php //IF INPUT VALIDATES, DATABASE UPDATE
	...
	if ((isset($_POST['pswrd0']) && $_POST['pswrd0']) || (isset($_POST['pswrd1']) && $_POST['pswrd1']) || (isset($_POST['pswrd2']) && $_POST['pswrd2'])) {
		...
		echo "Trying to change password";
	}
	...
	$possessive = $user_alias . "'s";
	
	if ($is_self) { $possessive = "Your"; }
				...
				{ 
					?><section><h1>User "<?=$user_alias?>" does not exist.</h1></section><?php 
				}
...
<?php
					...
					$post_query = "SELECT * FROM Questions WHERE post_user_id = '$user_id'";
					$post_result = $db->query($post_query);
					if ($post_result->num_rows == 0)
					{
						?><section><h1>"<?=$user_alias?>" has no posted questions</h1></section><?php 
					}
							...
							?>
								<div class="userpost-comment">
									<div class="userpost-comment-left">
										<h3> <img src="<?=$user_image?>" height="32" alt="Profile Picture"> From "<?=$user_alias?>"</h3>
